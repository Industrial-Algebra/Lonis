...
- **`block-v1.json` is composed**: `cargo run -p lonis-schema --example
  compose_block_schema` rebuilds it from the per-kind files (hoisting nested
  `$defs` like `plan_step` / `evidence_data`, with collision detection).
